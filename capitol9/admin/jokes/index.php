...
require_once $_SERVER['DOCUMENT_ROOT'].'/teste_ijdb/includes/access.inc.php';
...
if (!userHasRole('Content Editor'))
	{
		$error='Only Content Editors may acces this page.';
...
$result = mysqli_query($link, 'SELECT id, joketext, authorid FROM joke');
if (!$result)
  {
...
while ($row = mysqli_fetch_array($result))
  {
	$jokes[] = array('id' => $row['id'], 'text' => $row['joketext'], 'authorid' => $row['authorid']);
  }
